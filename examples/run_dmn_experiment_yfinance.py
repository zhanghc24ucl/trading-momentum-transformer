...
import pandas as pd
from settings.default import YFINANCE_TICKERS
from settings.fixed_params import MODLE_PARAMS
from mom_trans.backtest import run_all_windows
import numpy as np
from functools import reduce

# Stock sectors mapping (you can customize this based on your tickers)
TEST_MODE = False
ASSET_CLASS_MAPPING = dict(zip(YFINANCE_TICKERS, ["STOCKS"] * len(YFINANCE_TICKERS)))
TRAIN_VALID_RATIO = 0.90
TIME_FEATURES = False
FORCE_OUTPUT_SHARPE_LENGTH = None
EVALUATE_DIVERSIFIED_VAL_SHARPE = True
NAME = "experiment_yfinance_stocks"
    ...
    for v in versions:
        ...
        if changepoint_lbws:
            features_file_path = os.path.join(
                "data",
                f"yfinance_cpd_{np.max(changepoint_lbws)}lbw.csv",
            )
        else:
            features_file_path = os.path.join(
                "data",
...
if __name__ == "__main__":
    ...
    def get_args():
        ...
        parser = argparse.ArgumentParser(description="Run DMN experiment on yfinance data")
        parser.add_argument(
            "experiment",
            metavar="c",
            type=str,
        ...
        parser.add_argument(
            ...
            type=int,
            nargs="?",
            default=2000,  # Changed from 1990 for stocks
